...
127:        .Cells(1, 1).Value = "Title of the Book"
128:        .Cells(1, 2).Value = "Name of the Sheet"
...
135:                .Cells(LastRow, 2).Value = "Mistake!"
...
151:    Select Case Err.Number
        Case Else
153:            Call MsgBox("Error in DeletePaswortSheets" & vbLf & Err.Number & vbLf & Err.Description & vbCrLf & "in the line" & Erl, vbOKOnly + vbCritical, "Mistake:")
...
181:                ActiveSheet.Cells(LastRow, 3).Value = "the password has been removed from the sheet"
182:                ActiveSheet.Cells(LastRow, 3).Interior.Color = 13434828
183:            Else
184:                ActiveSheet.Cells(LastRow, 2).Value = .GetSheetNameFromId(CStr(i - 1))
185:                ActiveSheet.Cells(LastRow, 3).Value = "there was no password"
...
194:            ActiveSheet.Cells(LastRow, 3).Value = "the password has been removed from the structure of the book"
195:            ActiveSheet.Cells(LastRow, 3).Interior.Color = 3381555
196:        Else
197:            ActiveSheet.Cells(LastRow, 2).Value = sGetFileName(sfileName)
198:            ActiveSheet.Cells(LastRow, 3).Value = "there was no password on the structure of the book"
...
206:    Select Case Err.Number
        Case Else
208:            Call MsgBox("Error in XMLFileDelNodes" & vbLf & Err.Number & vbLf & Err.Description & vbCrLf & "in the line" & Erl, vbOKOnly + vbCritical, "Mistake:")
...
224:        Call MsgBox("Close the file for processing!", vbCritical, "Mistake:")
225:        Exit Sub
226:    End If
227:
228:    Call WriteBinFileVBAProject(sFileNameFull)
229:
230:    Call MsgBox("Deleting the Univable password is over" & vbNewLine & "Open the file", vbInformation, "Removing an Unwiewable Password:")
231:    Exit Sub
errMsg:
233:    Select Case Err.Number
        Case Else
235:            Call MsgBox("Error in DelPasswordVBAProjectUnivable" & vbLf & Err.Number & vbLf & Err.Description & vbCrLf & "in the line" & Erl, vbOKOnly + vbCritical, "Mistake:")
...
256:    Select Case Err.Number
        Case Else
258:            Call MsgBox("Error in WriteBinFileVBAProject" & vbLf & Err.Number & vbLf & Err.Description & vbCrLf & "in the line" & Erl, vbOKOnly + vbCritical, "Mistake:")
...
308:                Call MsgBox("Remove the password from the VBA project!", vbCritical, "The file is protected:")
309:                Exit Sub
310:            End If
311:    End Select
312:
...
422:    Select Case Err.Number
        Case Else
424:            Call MsgBox("Error in AddModule" & vbLf & Err.Number & vbLf & Err.Description & vbCrLf & "in the line" & Erl, vbOKOnly + vbCritical, "Mistake:")
...
446:            Err.Clear
447:        Case Else
448:            Call MsgBox("Error in AddModuleToProject" & vbLf & Err.Number & vbLf & Err.Description & vbCrLf & "in the line" & Erl, vbOKOnly + vbCritical, "Mistake:")
