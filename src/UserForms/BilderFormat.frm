...
' Date           : 15.09.2019
' ќбратна€ св€зь : info@VBATools.ru
...
165:    Txt = Me.txtValue    ' читаем текст из пол€ (дл€ недопущени€ ввода двух и более зап€тых)
166:    Select Case KeyAscii
        Case 8:    ' нажат Backspace - ничего не делаем
168:        Case 44: KeyAscii = IIf(InStr(1, Txt, ",") > 0, 0, 44)    ' если зап€та€ уже есть - отмен€ем ввод символа
169:        Case 46: KeyAscii = IIf(InStr(1, Txt, ",") > 0, 0, 44)    ' замен€ем при вводе точку на зап€тую
170:        Case 48 To 57    ' если введена цифра  - ничего не делаем
171:        Case Else: KeyAscii = 0    ' иначе отмен€ем ввод символа
172:    End Select
173: End Sub
...
195:    arrFormatDateCustomDiscription = Array("Date component separator. Some language standards may use other characters to represent the date component separator. This separator separates the day, month, and year when the date values are formatted. The symbol used as the separator of the date components in the formatted output data is determined by the system parameters.", _
                ...
                "Selected. days using abbreviations (SunЦSat). Localized.", _
                "Selected. of the day using the full name (SundayЦSaturday). Localized.", _
                ...
                "Selected. the abbreviated name of the month (JanЦDec). Localized.", _
                "Selecting the full name of the month (JanuaryЦDecember). Localized.", _
